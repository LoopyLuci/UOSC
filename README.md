...
- A *general, dynamically extensible* syscall ABI, a process model, a
  ...
  substantial, separate engineering, not a checkbox. SMP has a real first
  step (ACPI/MADT CPU topology discovery — see the table above and
  `kernel-x86_64/STATUS.md`) but nothing beyond it: no AP bring-up, no
  per-CPU state, no code has ever run on a second core. Six of these
  ...
  *do* now exist for real — see the table above — but each narrowly: a
  ...
  general, dynamically extensible syscall ABI and a real process model are
  still on this list; SMP has real CPU topology discovery now but is
  otherwise still on this list too — see above.)
...
│   └── README.md
├── kernel-x86_64/                # REAL: bootable kernel binary, boots in QEMU (BIOS+UEFI), 20/20 self-test
│   └── STATUS.md
